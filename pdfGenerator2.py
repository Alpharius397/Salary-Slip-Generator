...
def extract_data():
    ...
    for row in sheet.iter_rows(min_row=10):  # Adjust min_row as per your data's starting row
        if str(row[0].value) == student_id:  # Adjust column index as per your data's ID column
...
def generate_pdf(student_data):
    ...
    data = [['Student ID', 'Name', 'Subject', 'Marks'], student_data]  # Adjust headers and data as per your Excel structure
...
label_id = tk.Label(window, text="Enter Employee ID:")
label_id.pack()
...
text_excel = scrolledtext.ScrolledText(window, width=70, height=15)
