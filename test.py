...
class Database():
    ...
    def createData(self,month:str,year:int,columns:list[str]) -> int:
        cursor = self.db.cursor()

        try:
            sql = f"CREATE TABLE {month}_{year}({','.join([ col + ' VARCHAR(225) PRIMARY KEY' if col=='HR_EMP_CODE' else col + ' VARCHAR(225)' for col in columns])})"
            cursor.execute(sql)
    ...
    def updateData(self,data,month:str,year:int) -> None:
        cursor = self.db.cursor()
        ...
        for i in data['HR_EMP_CODE']:
            ...
            try:
                cursor.execute(f"INSERT INTO {month}_{year} ({','.join(new.keys())}) VALUE ({','.join([f"'{i}'" for i in new.values()])})")

            except mysql.connector.errors.ProgrammingError as e:
                cursor.execute(f'UPDATE {month}_{year} SET {query} WHERE HR_EMP_CODE={i}')
            except:
    ...
    def showTables(self) -> list[tuple[str]]:
        ...
        year =  [table[0].split('_') for table in temp if contains(table[0],'_')]
        
        for i,j in year:
            memo[j] += [i]
        cursor.execute("desc may_2024")
        print(cursor.fetchall()[:,1])
        return memo

    def getColumns(self,month,year):
        cursor = self.db.cursor()

        try:
            cursor.execute(f'desc {month}_{year}')
        except:
            print('MySQL Error Occured!')
            return None
    ...
    def fetchThat(self,month:str,year:int,emp_id:int) -> tuple[list[str]]:
        cursor = self.db.cursor()

        try:
            cursor.execute(f" SELECT * FROM {month}_{year} where HR_EMP_CODE={emp_id}")
        except:
            print('MySQL Error Occured!')
            return None
        
        columns = self.getColumns(month,year)
        return pd.DataFrame(cursor.fetchall(),columns=columns)

    # fetches all data from table month_year  
    def fetchAll(self,month:str,year:int) -> dict[str,list[str]]:
        cursor = self.db.cursor()

        try:
            cursor.execute(f"SELECT * FROM {month}_{year}")
