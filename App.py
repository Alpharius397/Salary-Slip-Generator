...
import pyperclip
import pandas as pd
from database import dataRefine,Database
...
class ExcelFunc():
    ...
    def load_database(self,month,year,chosen,type):
        db = Database(host=self.outer.cred['host'],user=self.outer.cred['user'],password=self.outer.cred['pass'],database=self.outer.cred['db'])
...
class App():
    def __init__(self):
        ...
        self.cred = defaultdict(lambda : None)
        self.children = {'setup':self.DataSet(self,self.app),'login':self.Login(self,self.app),'fileinput':self.FileInput(self,self.app),'landing':self.Landing(self,self.app),'interface':self.Interface(self,self.app),'DB':self.DBFetch(self,self.app),'upload':self.DBUpload(self,self.app)}
    ...
    class DataSet(BaseTemplate):
        def __init__(self,outer,master):
            ...
            self.host= ctk.CTkEntry(master=self.frame, placeholder_text="host", text_color=custom_color_scheme["text_color"], font=("Helvetica", 16))
            self.host.pack(pady=12, padx=10)

            self.user = ctk.CTkEntry(master=self.frame, placeholder_text="user", text_color=custom_color_scheme["text_color"], font=("Helvetica", 16))
            self.user.pack(pady=12, padx=10)

            self.passwrd = ctk.CTkEntry(master=self.frame, placeholder_text="Password", show="*", text_color=custom_color_scheme["text_color"], font=("Helvetica", 16))
            self.passwrd.pack(pady=12, padx=10)

            self.db = ctk.CTkEntry(master=self.frame, placeholder_text="database", text_color=custom_color_scheme["text_color"], font=("Helvetica", 16))
            self.db.pack(pady=12, padx=10)

            next_page = ctk.CTkButton(master=self.frame, text='Continue', command=self.next, fg_color=custom_color_scheme["button_color"], font=("Helvetica", 16))
            next_page.pack(pady=12, padx=10)

            back_page = ctk.CTkButton(master=self.frame, text='Back', command=self.back, fg_color=custom_color_scheme["button_color"], font=("Helvetica", 16))
            back_page.pack(pady=12, padx=10)

        def back(self):
            self.outer.children['login'].appear()

        def next(self):
            self.outer.cred['host'] = self.host.get()
            self.outer.cred['user'] = self.user.get()
            self.outer.cred['pass'] = self.passwrd.get()
            self.outer.cred['db'] = self.db.get()

            if Database(host=self.outer.cred['host'],user=self.outer.cred['user'],password=self.outer.cred['pass'],database=self.outer.cred['db']).status:
    ...
    class Login(BaseTemplate):
        ...
        def login(self):
            ...
            if known_user == username and known_pass == password:
                ...
                self.outer.children['setup'].appear()
    ...
    class Landing(BaseTemplate):
        def __init__(self,outer,master):
            ...
            button_back= ctk.CTkButton(master=self.frame , text='Back', command=self.back, fg_color=custom_color_scheme["button_color"], font=("Helvetica", 16))
        ...
        def preview(self):
            self.outer.children['interface'].available_data()
            self.outer.children['interface'].changeType(event=None)
    ...
    class Interface(BaseTemplate,ExcelFunc):
        ...
        def available_data(self):
            data = Database(host=self.outer.cred['host'],user=self.outer.cred['user'],password=self.outer.cred['pass'],database=self.outer.cred['db']).showTables().get(self.chosen.get().lower())
    ...
    class DBUpload(BaseTemplate,ExcelFunc):
        ...
        def deload(self):
            database = Database(host=self.outer.cred['host'],user=self.outer.cred['user'],password=self.outer.cred['pass'],database=self.outer.cred['db'])
            if self.year.get():
                try:
