...
class PDFTemplate:
    ...
    def _load_defaults(
        self, path: Path, fileType: Literal["json", "html"], **kwargs: str
    ):
        finalStatus = False
        for filename, data in kwargs.items():
            status, msg = self.make_file(
                Path(path, f"{filename.replace('_', '.').strip(' .')}.{fileType}"), data
            )
    ...
    def _load_template(
        self, file_name: Path, dataFrame: dict[str, dict]
    ) -> tuple[bool, str]:
        status = False
        ...
        try:
            with pd.ExcelWriter(file_name, mode="w") as writer:
                for (
                    sheet,
                    sheetData,
                ) in dataFrame.items():  # safe cause default constant
                    pd.DataFrame.from_dict(sheetData).to_excel(
    ...
    def _read_excel_to_dict(self, file: Path) -> None:
        ...
        memo: dict[str, dict[str | Hashable, list[str]]] = {}
    ...
    def make_file(
        self, file_name: Path, default: Optional[str] = None
    ) -> tuple[bool, str]:
        status = False
        msg = f"File Created: {file_name}"
