...
class BaseTemplate:
    ...
    def can_start_thread(self) -> bool:
        ...
        can_we_start = bool(
            ...
            or (self.process and (not self.process.is_alive()))
            and (
    ...
    def get_widgets_to_disable(self):
        ...
        for attr in all_attribute:
            ...
            if (
                ...
                or isinstance(widget, ctk.CTkOptionMenu)
                or isinstance(widget, ctk.CTkEntry)
            ):
                yield widget
        ...
        for sheet in TEMPLATE_SHEET:
            ...
            if ((name := mapping(columns, _name)) is not None) and (
                ...
                memo[sheetName], tempDict = TemplateGenerator.make_everything(
                    {
                        row[column_memo[name]]: row[column_memo[title]]
                        for row in row_data
                    }
                )
...
class SendMail(BaseTemplate):
    ...
    def browse_file(self) -> None:
        ...
        file_path = filedialog.askopenfilename(
            filetypes=[("PDF Files", ".pdf")], initialdir=Path(APP_PATH).parent
        )
...
class SendBulkMail(BaseTemplate):
    ...
    def send_mail(self) -> None:
        ...
        code_col = mapping(BaseTemplate.data.columns, "HR EMP CODE")  # type: ignore
        email_col = None
        all_columns = BaseTemplate.data.columns  # type: ignore
        ...
        if self.can_start_thread():
            self.process = Process(
                target=MailingWrapper().change_state(month, year).massMail,
                kwargs={
    ...
    def table_exists(self):
        ...
        if self.can_start_thread():
            self.process = Process(
                target=DatabaseWrapper(**self.outer.CRED).get_data,
                kwargs={
                    "queue": self.QUEUE,
                    "institute": self.chosen_institute.get(),
                    "type": self.chosen_type.get(),
...
class Interface(BaseTemplate):
    ...
    def check_database_thread(self):
        ...
        if table:
            self.outer.CHILD[DataPreview.__name__].tables = table  # type: ignore
    ...
    def preview(self) -> None:
        if self.can_start_thread():
            self.process = Process(
                target=DatabaseWrapper(**self.outer.CRED).check_table,
    ...
    def delete_database_thread(self):
        ...
        if table:
            self.outer.CHILD[DataPeek.__name__].tables = table  # type: ignore
...
class DataPreview(BaseTemplate):
    ...
    def check_database_thread(self) -> None:
        ...
        if table is not None:
            GUI_Handler.view_excel(
                table,
                self.outer.CHILD[DataView.__name__].text_excel,  # type: ignore
            )  # type: ignore
            ...
            if htmls and jsons:
                GUI_Handler.setOptions(
                    jsons,
                    self.outer.CHILD[DataView.__name__].json_list,  # type: ignore
                    self.outer.CHILD[DataView.__name__].json,  # type: ignore
                )  # type: ignore
                GUI_Handler.setOptions(
                    htmls,
                    self.outer.CHILD[DataView.__name__].html_list,  # type: ignore
                    self.outer.CHILD[DataView.__name__].html,  # type: ignore
                )  # type: ignore
...
class DataView(BaseTemplate):
    ...
    def decrease_size(self) -> None:
        ...
        if self.size == MIN_TEXT_SIZE:
            GUI_Handler.lock_gui_button([self.minus])
    ...
    def increase_size(self) -> None:
        ...
        if self.size == MAX_TEXT_SIZE:
            GUI_Handler.lock_gui_button([self.plus])
    ...
    def copy_to_clipboard_thread(self, emp_id: str) -> None:
        ...
        search_result = BaseTemplate.data[
            BaseTemplate.data[str(self.id_column)] == emp_id
        ]  # type: ignore
    ...
    def copy_row_to_clipboard(self) -> None:
        ...
        if emp_id:
            if self.can_start_thread():
                self.thread = Thread(
                    target=self.copy_to_clipboard_thread,
    ...
    def single_print_pdf_cover(self) -> None:
        ...
        try:
            _file = filedialog.asksaveasfile(
                ...
                initialdir=Path(APP_PATH).parent,
        ...
        if file:
            ...
            if self.can_start_thread():
                ...
                self.process = Process(
                    target=PandaWrapper(
                        BaseTemplate.data,  # type: ignore
                        str(self.id_column),
                        PDF_TEMPLATE,  # type: ignore
                    ).litany_of_scroll,
                    kwargs={
                        "queue": self.QUEUE,
...
class FileInput(BaseTemplate):
    ...
    def _go_to_upload_thread(self):
        GUI_Handler.view_excel(
            BaseTemplate.data[self.sheet.get()],  # type: ignore
            self.outer.CHILD[UploadData.__name__].text_excel,  # type: ignore
        )  # type: ignore
        self.outer.CHILD[UploadData.__name__].sheet = self.sheet.get()  # type: ignore
    ...
    def next_row(self):
        max_row = BaseTemplate.data[self.sheet.get()].shape[0]  # type: ignore

        self.row_index = min(max_row, self.row_index + 1)

        GUI_Handler.changeText(self.row, str(self.row_index))
        if self.row_index == max_row:
            GUI_Handler.lock_gui_button([self.row_plus])
        else:
    ...
    def prev_row(self):
        ...
        if self.row_index == 0:
            GUI_Handler.lock_gui_button([self.row_minus])
        else:
        ...
        if file_path:
            if self.encryption and password:
                if self.can_start_thread():
                    ...
                    self.process = Process(
                        target=Decryption.fetch_encrypted_file,
                        kwargs={
                            "queue": self.QUEUE,
                ...
                if self.can_start_thread():
                    self.thread = Thread(target=self.change_row_thread, daemon=True)
                    self.process = Process(
                        target=Decryption.fetch_decrypted_file,
                        kwargs={
                            "queue": self.QUEUE,
                            "file_path": Path(file_path),
                            "skip": self.row_index,
    ...
    def decrease_size(self):
        ...
        if self.size == MIN_TEXT_SIZE:
            GUI_Handler.lock_gui_button([self.size_minus])
    ...
    def increase_size(self) -> None:
        ...
        if self.size == MAX_TEXT_SIZE:
            GUI_Handler.lock_gui_button([self.size_plus])
    ...
    def select_file(self) -> None:
        file_path = filedialog.askopenfilename(
            filetypes=[("Excel Files", ".xlsx;.xls")], initialdir=Path(APP_PATH).parent
        )
        ...
        if file_path:
            ...
            if self.can_start_thread():
                ...
                self.process = Process(
                    target=Decryption.is_encrypted_wrapper,
                    kwargs={"queue": self.QUEUE, "file_path": Path(file_path)},
                    daemon=True,
                )
                self.thread.start()
                self.process.start()
    ...
    def load_decrypted_file(self):
        ...
        if file_path:
            if self.can_start_thread():
                self.thread = Thread(
                    target=self.load_unprotected_data_thread, daemon=True
                )
                self.process = Process(
                    target=Decryption.fetch_decrypted_file,
                    kwargs={
    ...
    def load_encrypted_file(self):
        ...
        if file_path:
            ...
            if self.can_start_thread():
                self.thread = Thread(
                    target=self.load_protected_data_thread, daemon=True
                )
                self.process = Process(
                    target=Decryption.fetch_encrypted_file,
                    kwargs={
    ...
    def change_view_thread(self):
        ...
        current_data = BaseTemplate.data[current_sheet]  # type: ignore

        GUI_Handler.view_excel(current_data, self.text_excel)  # type: ignore
        GUI_Handler.unlock_gui_button(self.to_disable)

    def change_row_thread(self):
        GUI_Handler.lock_gui_button(self.to_disable)
        GUI_Handler.place_before(self.text_excel, self.quit)  # type: ignore
        current_sheet = self.sheet.get()
        ...
        GUI_Handler.view_excel(BaseTemplate.data[self.sheet.get()], self.text_excel)  # type: ignore
        GUI_Handler.unlock_gui_button(self.to_disable)
        GUI_Handler.remove_widget(self.quit)
...
class UploadData(BaseTemplate):
    ...
    def create_in_db(self):
        ...
        if year_check(year):
            if self.can_start_thread() and messagebox.askyesnocancel(
                ...
                self.process = Process(
                    target=DatabaseWrapper(**self.outer.CRED).create_table,
                    kwargs={
                        "queue": self.QUEUE,
                        "institute": institute,
                        "type": type,
                        "year": year,
                        "month": month,
                        "data_columns": list(BaseTemplate.data[self.sheet].columns),  # type: ignore
                    },
                    daemon=True,
                )  # type: ignore
                self.thread.start()
    ...
    def update_in_db(self):
        ...
        month = self.chosen_month.get()
        year = self.entry_year.get()
        data = BaseTemplate.data[self.sheet]  # type: ignore
        ...
        if year_check(year):
            if self.can_start_thread() and messagebox.askyesnocancel(
                ...
                self.process = Process(
                    target=DatabaseWrapper(**self.outer.CRED).fill_table,
                    kwargs={
                        "data": data,
                        "queue": self.QUEUE,
                        "institute": institute,
    ...
    def delete_from_db(self):
        ...
        if year_check(year):
            if self.can_start_thread() and messagebox.askyesnocancel(
                ...
                self.process = Process(
                    target=DatabaseWrapper(**self.outer.CRED).delete_table,
                    kwargs={
                        "queue": self.QUEUE,
                        "institute": institute,
                        "type": type,
...
class TemplateInput(BaseTemplate):
    ...
    def generate_template(self):
        ...
        if self.can_start_thread():
            self.process = Process(
                target=TemplateGenerator.make_template,
                kwargs={
            ...
            self.thread = Thread(
                target=self.go_to_generate_template_thread, daemon=True
            )
            self.process.start()
            self.thread.start()
        else:
    ...
    def load_unprotected_data_thread(self) -> None:
        ...
        try:
            ...
            if (sheets) and (data is not None):
                ...
                if not all(
                    [
                        checkColumns(_data.columns, TEMPLATE_COLUMN)  # type: ignore
...
class TemplateGeneration(BaseTemplate):
    ...
    def generate_template(self) -> None:
        ...
        if self.can_start_thread():
            self.process = Process(
                target=TemplateGenerator.make_excel,
                kwargs={
                    "file_name": Path(file),
                    "data": BaseTemplate.data,
                    "queue": self.QUEUE,
    ...
    def next_row(self):
        ...
        GUI_Handler.changeText(self.row, self.row_index)  # type: ignore
        if self.row_index == max_row:
            GUI_Handler.lock_gui_button([self.row_plus])
        else:
            self.get_data()
    ...
    def load_unprotected_data_thread(self) -> None:
        ...
        try:
            ...
            if (sheets) and (data is not None):
                ...
                if not all(
                    [
                        checkColumns(list(_data.columns), TEMPLATE_COLUMN)
                        for _data in data.values()
                    ]
                ):  # type: ignore
                    messagebox.showwarning(
