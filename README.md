...
- Securely store all data into local Sqlite database
...
**_NOTE: There is a pre-compiled sqlite3.lib and sqlite3.h in the bin folder_**

### wkhtmltopdf dependency
Install wkhtmltopdf and place the wkhtmltopdf.exe into the bin folder

### Creds addition

In the main folder, add the following file: **src/creds.py** with following code

```python
from src.dataTypes import SMTP_CRED
...
```bash
pyinstaller --onefile main.py --name App.exe --noconsole --add-binary "bin/wkhtmltopdf.exe;./bin" --add-data "doc;./doc" --add-data "html;./html" --add-data "json;./json" --add-data "images;./images" --add-data "excel;./excel"
```
